...
public abstract class AbstractCompletionProvider
	...
	public List<Completion> getCompletionByInputText(String inputText) {
		...
		System.out.println("getcompletionbyinputtext");
	...
	/*public List<Completion> getCompletionByInputText(String inputText) {
		...
		for (int i = 0; i < completions.size(); i++) {
			if (completions.get[i].startsWith(inputText)) {
				completions.get[i].setRelevance(completions.get[i].getRelevance()+100000);
				result.add(completions.get[i]);
			}
		}
	...
	protected List<Completion> getCompletionsImpl(JTextComponent comp) {
		...
		if (text!=null) {
			...
			for (int i = 0; i < completions.size(); i++) {
				...
				//Place completions beginning with the text to the top of the list by increasing the relevance
				/*if (completions.get(i).getInputText().toLowerCase().startsWith(text.toLowerCase())) {
					completions.get(i).setRelevance(completions.get(i).getRelevance()+100000);
					retVal.add(completions.get(i));
				} else if (completions.get(i).getInputText().toLowerCase().contains(text.toLowerCase())) {
					retVal.add(completions.get(i));
				}*/
